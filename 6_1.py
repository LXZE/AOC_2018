...
def process(dps, data, core, acc_result):
	...
	for x in range(0, dps.shape[0]):
		for y in range(0, max_y):
			point = dps[x][y]
			point_val = tocoord(point)

			tmp = ''
			if point_val[0] == 0 or point_val[1] == 0 or point_val[1] == max_y-1 or point_val[0] == max_x-1:
				tmp += 'x'

			dist = {}
			for (idx, coord) in data:
				dist[idx] = d(coord, point_val)
			min_val = dist[min(dist.keys(), key=lambda k: dist[k])]

			selected = {k: v for k, v in dist.items() if v == min_val}
			if len(selected) > 1:
				if tmp == '':
					tmp = '.'
			else:
				tmp += str(list(selected.keys())[0])
			res[x][y] = tmp
...
for idx in range(len(data)):
	if 'x'+str(idx) not in counter:
		count[idx] = counter[str(idx)]
print(max(count))